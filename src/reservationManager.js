...
module.exports = {
    reserveMatch: function (timeString, userId, userName) {
        ...
        if (runningMatch) {
            return {
                text: 'Sorry, um ' + time.format('HH:mm') + ' Uhr ist der Raum bereits von ' + this.getUserObject(runningMatch.createdBy) + ' belegt!',
                attachments: [{
    ...
    getMatchList: function () {
        ...
        list += reservedMatches.map((match, index) => {
            return '\n' + (index + 1) + '. ' + match.time.format('HH:mm') + ' Uhr, Teilnehmer:  ' +
                match.players.map(player => this.getUserObject(player)).join(' ')
        });
    ...
    cancelMatch: function (matchId, userId) {
        ...
        if (match.createdBy.userId === userId) {
            ...
            return {
                text: 'Hey ' + match.players.map(player => this.getUserObject(player)).join(', ') +
    ...
    joinMatch: function (matchId, userId, userName) {
        ...
        if (match.players.length === MAX_PLAYER) {
            ...
            text += 'Teilnehmer: ' + match.players.map(player => this.getUserObject(player)).join(', ') + ', '
            text += this.getUserObject({
                userName,
                userId
            }) + ' \n';
    ...
    checkTimeString: function (timeString) {
        return !/[0-2]?[0-9]:[0-5][0-9]/.test(timeString);
    },
    getRunningMatch: function (requiredMatchTime) {
    ...
    getFreeSlots: function () {
        let nextSlot = this.nearestFutureMinutes(20, moment());
        let freeSlots = [];
        let time;
    ...
    nearestFutureMinutes: function (interval, someMoment) {
